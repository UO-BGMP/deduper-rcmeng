...
parser = argparse.ArgumentParser(description="Given a sorted SAM file, this program removes all of the PCR duplicates present and returns the file, in it's entirety, minus the duplicates.")
parser.add_argument("-f", "--file_path", help="Requires the path to the sorted SAM file that needs to be deduped.")
parser.add_argument("-u", "--UMI" ,help="This flag indicates that known UMIs (not randomers) are used in the SAM file and requires an UMI file to be passed to it.")
...
with open(args.file_path) as fh:
    with open(args.file_path + "_deduped", 'w') as fh2:
        #Establishing the dictionary that will be used to store all the reads for comparison and setting counters for PCR Duplicates, and reads with incorrect UMIs.
        testdict = {}
        PCRDuplicates = 0
        BadUmi = 0
                    ...
                    if (umi2,test) in testdict.items():
                        ...
                        if re.search('^\d+[S]', line2[5]):
                            ...
                            if (umi2, test2) in testdict.items():
                                ...
                                fh2.write(line)
                                testdict[umi2] = [strand, chr, str(newstartingposition)]
                        else:
                            fh2.write(line)
